// Bruit de simplex (d'après Stefan Gustavson) avec graine, et utilitaires de hasard.
...
// Hachage entier -> [0, 1) déterministe.
...
// Convertit un texte (graine saisie par le joueur) en entier 32 bits.
...
// Bruit fractal (somme d'octaves), normalisé approximativement dans [-1, 1].
...
export function fbm3(noise, x, y, z, octaves, lacunarity = 2, gain = 0.5) {
  let sum = 0, amp = 1, freq = 1, norm = 0;
  for (let i = 0; i < octaves; i++) {
    sum += noise.noise3(x * freq, y * freq, z * freq) * amp;
    norm += amp;
    amp *= gain;
    freq *= lacunarity;
  }
